...
export const supabase: Handle = async ({ event, resolve }) => {
  ...
  event.locals.getUser = async (code: string | null = null) => {
    const getExistingUserOrCreateAnonymousUser = async (
      code: string | null,
    ) => {
      const {
      ...
      const { data: newUser, error: eNewUser } =
        await event.locals.supabase.auth.signInAnonymously().then((r) => {
    ...
    const getExistingProfileOrCreateNewProfile = async (user: User) => {
      ...
      if (eProfile) {
        console.error(
          `Error retrieving profile: ${JSON.stringify(eProfile, null, 2)}`,
        )
      }
      ...
      if (eProfile2) {
        console.error(
          `Error making new profile: ${JSON.stringify(eProfile2, null, 2)}`,
        )
      }
