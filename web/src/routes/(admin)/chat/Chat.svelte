  ...
  supabase
    ...
    .on(
      ...
      (payload) => {
        ...
        if (payload.new.name === "user" || payload.new.event_type === "tool")
          return
...
<div
  class="items-between flex flex-col justify-start gap-2 overflow-y-scroll px-2 pt-2"
>
  {#each shownEvents as msg, i (i)}
    {#if msg != null}
      <div
        class={`flex my-2 ${msg.name == "user" ? "ml-16 justify-end" : "mr-16 justify-start"}`}
      >
        {#if msg.name != "user"}
        ...
        {/if}
        <p
          class="prose prose-main text-white rounded-lg border border-primary/30 {msg.name ==
          'user'
            ? 'bg-primary-container'
            : ' bg-primary-foreground'} px-2 py-1 text-primary-container-on marker:text-primary"
        >
